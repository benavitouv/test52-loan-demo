...
const WEBHOOK_URL = getEnv(
  'WEBHOOK_URL',
  'https://api.demo.wonderful.ai/api/v1/tasks/webhook/45846a26-7248-408b-864f-cce82cfda859'
);
const WEBHOOK_SECRET = getEnv(
  'WEBHOOK_SECRET',
  'bf4549f6-d316-4f1c-92e5-d678cf1a1c91'
);
...
const triggerWebhook = async ({ email, firstName, lastName, attachmentId }) => {
  const subject = `Loan Application - ${firstName} ${lastName}`.trim();
  ...
  const webhookResponse = await fetch(WEBHOOK_URL, {
    ...
    body: JSON.stringify({
      ...
      payload: {
        ...
        subject,
        message: 'Process this loan application',
  ...
  if (req.method === 'POST' && url.pathname === '/api/submit') {
    try {
      ...
      if (!file || typeof file === 'string') {
        jsonResponse(res, 400, { ok: false, error: 'missing_file', message: 'A document attachment is required.' });
        return;
      }

      const attachmentId = await uploadAttachment(file);
      const webhookResult = await triggerWebhook({ email, firstName, lastName, attachmentId });
...
server.listen(PORT, () => {
  console.log(`Loan Application UI server running at http://localhost:${PORT}`);
});
